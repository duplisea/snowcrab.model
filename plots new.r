...
##############################################################
plots.f= function(dataset,plot.type=1,general.type="abundance", years.removed=100){
...
# general.type = the general kind of plot produced:
  # "abundance" straight abundance of stages or groups
  # "percent" % abundance of stages or groups
  # "sex" sex ratio of TM males to TM females
  # "size" caparace width of TM males in sea and in the catch
  # "power" power spectrum and autocorrelation
...
if (general.type=="percent"){
  if (plot.type==1){
    #1=proportion of TM males which are legal to fish
    data.to.plot= t(data)%*%stage.binaries.f("fishable.males")
    data.to.plot1= (data.to.plot[seq(2,length(data.to.plot),by=2)]+data.to.plot[seq(2,length(data.to.plot),by=2)-1])/2
    data.to.plot= t(data)%*%stage.binaries.f("TM.males")
    data.to.plot2= (data.to.plot[seq(2,length(data.to.plot),by=2)]+data.to.plot[seq(2,length(data.to.plot),by=2)-1])/2
    prop.fishable= round(data.to.plot1 / data.to.plot2, 2)
  ...
  if (plot.type==2){
    #2 = proportion of fishable (legal) population which was fished
    data.to.plot= t(data)%*%stage.binaries.f("fishable.males")
    data.to.plot1= (data.to.plot[seq(2,length(data.to.plot),by=2)]+data.to.plot[seq(2,length(data.to.plot),by=2)-1])/2
    fished.data= dataset$catch[-(1:years.removed),]
    total.catch= apply(fished.data,1,sum)
    prop.fished= round(total.catch / data.to.plot1, 2)
  ...
  if (plot.type==3){
    #3 = proportion of TM males population which were fished
    data.to.plot= t(data)%*%stage.binaries.f("TM.males")
    data.to.plot1= (data.to.plot[seq(2,length(data.to.plot),by=2)]+data.to.plot[seq(2,length(data.to.plot),by=2)-1])/2
    fished.data= dataset$catch[-(1:years.removed),]
    total.catch= apply(fished.data,1,sum)
  ...
  if (plot.type==4){
    #4 = proportion reproductive females which are multiparous
    ...
    prop.multi= round(t(pop.data)%*%multi.bin / (t(pop.data)%*%multi.bin + t(pop.data)%*%primi.bin),2)
...
##############################################################
sex.ratio.f= function(dataset,years.removed=100){
...
### ratio with legal TM males condition 2 to 4
  ...
  plot((half.years.removed+1):(length(sex.ratio)+half.years.removed),sex.ratio,
...
##############################################################
stage.by.stage.f= function(dataset,plot.instars=1:20,time.steps.removed=100){
...
##############################################################
TM.male.sizes.f= function(dataset,years.removed=100){
...
data= dataset$n.crab.6months[,-(1:half.years.removed)]
  ...
  plot(((half.years.removed+1):(length(data.to.plot)+half.years.removed))/2,mean.CW.sea,
  ...
  plot(((half.years.removed+1):(length(data.to.plot)+half.years.removed))/2,mean.wgt.sea,
