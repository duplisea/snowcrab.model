...
##############################################################
analysis$plots.f= function(dataset,plot.type=1,years.removed=10){
...
data.to.plot= t(data)%*%inputs$stage.binaries.f(stages)
...
plot((years.removed+1):(length(catch.data)+years.removed),catch.data,
  type="l",xlab="Simulation time step (year)", ylab="Catch (tonnes)",lty=1)
par(new = TRUE)
...
axis(4)
legend("topright",lty=c(1,2),legend=c("Catch","Expl. rate"),bty="n")
...
##############################################################
analysis$percent.plots.f= function(dataset,plot.type=1,years.removed=10){
...
if (plot.type==1){
  ...
  half.years.removed=years.removed*2
  pop.data= dataset$n.crab.6months[,-(1:half.years.removed)]
  TM.male.bin= inputs$stage.binaries.f("TM.males")
  fishable.male.bin= inputs$stage.binaries.f("fishable.males")
...
if (plot.type==4){
  ...
  primi.bin= inputs$stage.binaries.f("primiparous.females")
  multi.bin= inputs$tage.binaries.f("multiparous.females")
...
##############################################################
analysis$sex.ratio.f= function(dataset,years.removed=10){
...
### ratio with legal TM males condition 2 to 4
  male.abund= t(data)%*%inputs$stage.binaries.f("TM.mal.2to4.legal")
  fem.abund= t(data)%*%inputs$stage.binaries.f("TM.females")
...
### ratio with sublegal TM males condition 2 to 4
  ...
  male.abund= (t(data)%*%inputs$stage.binaries.f("TM.mal.2to4"))-male.abund
  fem.abund= t(data)%*%inputs$stage.binaries.f("TM.females")
  sex.ratio= logb(fem.abund/male.abund,10)
  plot((months.removed+1):(length(sex.ratio)+months.removed),sex.ratio,
...
### ratio with all TM males condition 2 to 4
  male.abund= t(data)%*%inputs$stage.binaries.f("TM.mal.2to4")
  fem.abund= t(data)%*%inputs$stage.binaries.f("TM.females")
  sex.ratio= logb(fem.abund/male.abund,10)
  plot((months.removed+1):(length(sex.ratio)+months.removed),sex.ratio,
...
##############################################################
analysis$stage.by.stage.f= function(dataset,plot.instars=1:20,time.steps.removed=10){
...
##############################################################
analysis$TM.male.sizes.f= function(dataset,years.removed=10){
...
###### Average size in the sea
  ...
  # only the instars of interest (1 in the binary matrix) and sums over them for a
  #time step and then plots over the simulation period
  fished.stages= inputs$stage.binaries.f(stages)
  data.to.plot= t(data)%*%fished.stages

  #caparapace width
  CW= inputs$stage.binaries.f("CW")*fished.stages
  ...
  wgt= inputs$stage.binaries.f("wgt")*fished.stages
